...
@jax.jit
def split_heads(hidden_states, num_heads, head_dim):
    return hidden_states.reshape(hidden_states.shape[:2] + (num_heads, head_dim))


@jax.jit
def merge_heads(hidden_states, embed_dim):
...
def quantize_input(hidden_states, gamma, eps=1e-6):
    gamma = 127 / hidden_states.abs().max(dim=-1, keepdim=True).values.clamp_(min=eps)
    quantized_x = (hidden_states * gamma).round().clamp_(-128, 127) / gamma
    return jnp.asarray(quantized_x, dtype=jnp.int8)

    
@jax.jit
def create_llama_running(config: LlamaConfig):
    kv_head_dim = config.hidden_dim // config.n_kv_heads
    ...
    def run_llama(
        ...
        for i in range(config.n_layers):
            ...
            k = jnp.repeat(k, num_key_value_groups, axis=2)
            v = jnp.repeat(v, num_key_value_groups, axis=2)
            ...
            attn_output = merge_heads(attn_output, config.dim)
